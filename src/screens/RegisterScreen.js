...
export default function RegisterScreen({ navigation }) {
  ...
  return (
    <ScreenContainer>
      ...
      <TextInputField label="Họ tên" value={form.fullName} onChangeText={(value) => setValue('fullName', value)} placeholder="Nguyễn Văn A" />
      <TextInputField label="Email" value={form.email} onChangeText={(value) => setValue('email', value)} autoCapitalize="none" keyboardType="email-address" placeholder="email@example.com" />
      <TextInputField label="Số điện thoại" value={form.phone} onChangeText={(value) => setValue('phone', value)} keyboardType="phone-pad" placeholder="09xxxxxxxx" />
      <TextInputField label="Mật khẩu" value={form.password} onChangeText={(value) => setValue('password', value)} secureTextEntry placeholder="Ít nhất 6 ký tự" />
